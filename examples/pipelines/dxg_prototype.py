...
xdex = app.marshal_external_data(
    ...
    # not being generated today. If we don't declare this and use 'xdex' signal in an event-based manner, it will cause
...
txdex = app.create_data(id="txdex",  # default DataFrame ID in downstream nodes (if not overwritten)
                        inputs=[xdex['NA']['*'].ref.range_check(True),
                                timer_signal_daily],
...
app.process(timer_signal_daily['2021-09-20'], target_route_id=txdex)

app.poll(txdex['NA']['2021-09-20'])

# app.execute(txdex['NA']['2021-09-20'])
