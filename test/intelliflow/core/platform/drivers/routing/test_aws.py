...
class TestAWSRoutingTableDriver(AWSTestBase, DriverTestUtils):
    ...
    params = {}
    mock_host_platform = None
    mock_routing_table = None
    test_route_record = None
    test_route = None
    ...
    def setup_platform_and_params(self):
        ...
        self.mock_routing_table = AWSDDBRoutingTable(self.params)
    ...
    def setup_test_route_record(self, mock_route=None, deactivation_timestamp=None, compute_resp=None):
        ...
        self.mock_routing_table._save(self.test_route_record)
    ...
    def test_mock_ddbroutingtable_successful_activate(self):
        ...
        self.mock_routing_table.dev_init(self.mock_host_platform)
        self.mock_routing_table.activate()
        self.setup_test_route_record()
        saved_record = self.mock_routing_table._load(self.test_route_record.route.route_id)
        assert self.check_route_record_equality(self.test_route_record, saved_record)

        assert self.mock_routing_table.routing_table_name == AWSDDBRoutingTable.ROUTES_TABLE_NAME_FORMAT.format(
            AWSDDBRoutingTable.__name__, self.mock_host_platform._context_id.lower(), self.params[CommonParams.REGION]
        )
        assert self.mock_routing_table.routing_history_table_name == AWSDDBRoutingTable.ROUTES_HISTORY_TABLE_NAME_FORMAT.format(
            AWSDDBRoutingTable.__name__, self.mock_host_platform._context_id.lower(), self.params[CommonParams.REGION]
        )
        self.patch_aws_stop()
    ...
    def test_mock_ddbroutingtable_sucessful_activate_twice_clear_all_routes(self):
        ...
        self.mock_routing_table.activate()
        self.setup_test_route_record()
        self.mock_routing_table.activation_strategy = RoutingActivationStrategy.ALWAYS_CLEAR_ACTIVE_ROUTES
        self.mock_routing_table.activate()
        saved_record = self.mock_routing_table._load(self.test_route_record.route.route_id)
    ...
    def test_mock_ddbroutingtable_successful_fetch_active_route_record(self):
        ...
        self.mock_routing_table.activate()
        self.setup_test_route_record()
        route_record_iterator = self.mock_routing_table.load_active_route_records()
    ...
    def test_mock_ddbroutingtable_successful_process_signals_with_new_signal(self):
        ...
        self.setup_test_route_record(mock_route=old_route, deactivation_timestamp=1608154579, compute_resp=compute_response)
        ...
        self.mock_routing_table._process_internal({old_route}, {new_route})
        assert not self.mock_routing_table.load_active_route_record(old_route.route_id)
        inactive_route_records_iterator = self.mock_routing_table.load_inactive_compute_records(old_route.route_id)
